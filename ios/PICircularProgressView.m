...
- (void)drawRect:(CGRect)rect
{
    ...
    if (_showText && _textColor)
    {
        ...
        progressString = [NSString stringWithFormat:@"%.0f%%", 100 - _progress * 100.0];
...
//        CGSize expectedSize =
        ...
        // 文字位置
        CGPoint origin = CGPointMake(center.x - expectedSize.width / 2.0, center.y - expectedSize.height / 1.5);
        [_textColor setFill];
        
        //[progressString drawAtPoint:origin forWidth:expectedSize.width withFont:font lineBreakMode:NSLineBreakByWordWrapping];
        [progressString drawAtPoint:origin withAttributes:@{NSFontAttributeName:font,NSForegroundColorAttributeName:_textColor}];
    }
